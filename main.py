...
def main() -> None:
    ...
    wallfollowing = Algorithms.WallFollowing(Maze)
    ...
    wallfollowing_Agent = agent(Maze, footprints=True, x=1, y=1, color=COLOR.yellow, shape="arrow")

    a_star_path = a_star.pathFinding()
    dfs_search_path, dfs_path = dfs.pathFinding()
    bfs_search_path, bfs_path = bfs.pathFinding()
    wallfollwoing_nodeadends, wallfollwoing_deadends = wallfollowing.pathFinding()
    ...
    Maze.tracePath({wallfollowing_Agent: wallfollwoing_deadends})
    ...
    # )
    textLabel(
        Maze, title="Wall following (Deadends): ", value=len(wallfollwoing_deadends) + 1
    )
    Maze.run()
